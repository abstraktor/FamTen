window.Model.Player = Backbone.Model.extend({
	...
	changed: function(){
		console.log("player "+ this.name + "has changed");
	},
	...
	validate: function(attrs){
		...
		if(attrs.stats!==undefined) {
			...
			[ this.wins, this.evens, this.loses ] = stats.map(function(x){return parseInt(x);});
		}
		delete attrs.stats;
	}
});
...
window.Collection.Players = Backbone.Collection.extend({
	...
	name_exists: function(name) {
		return this.any(function(p){return p.attributes.name == name});
	},
